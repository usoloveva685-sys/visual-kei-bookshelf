...
def extract_links(page, base_url, patterns=None):
    ...
    for href, anchor in re.findall(
        ...
        if patterns and not any(re.search(p, href, re.I) for p in patterns):
            continue
...
def megaknigi(query, limit=10):
    ...
    # У MegaKnigi текущий адрес формы поиска не закреплён в нашей старой версии.
    # Поэтому пробуем несколько распространённых вариантов, а затем разбираем
    # найденные страницы книг без привязки к старому /read/ пути.
    ...
    for search_url in candidates:
        ...
        # Если сайт не использует очевидный путь, берём ссылки с названиями
        # из области результатов как запасной вариант.
        if not links:
            links = extract_links(page, final_url)

        for url in links[:40]:
            if any(x in url.lower() for x in ("/cat/", "/search", "/find", "/user", "/login")):
                continue

            item = parse_generic_product(url, "MegaKnigi")
            if not item:
                continue
...
def authortoday(query, limit=10):
    ...
    for url in links[:40]:
        item = parse_generic_product(url, "Author.Today")
        if not item:
            continue
...
def chitai_gorod(query, limit=10):
    ...
    # Текущий поиск Читай-города использует параметр phrase.
    search_url = CHITAI_BASE + "/search?" + urllib.parse.urlencode({"phrase": query})

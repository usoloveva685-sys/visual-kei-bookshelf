...
def external_site_search(query, domains, limit=20):
    """Запасной поиск через Bing, ограниченный нужными доменами.
    Нужен на случай, когда сайт не отдаёт свой поисковый HTML серверу Render.
    """
    q = " ".join(f"site:{d}" for d in domains) + " " + query
    url = "https://www.bing.com/search?" + urllib.parse.urlencode({"q": q, "count": 20})

    try:
        _, final_url, page = fetch_text(url, timeout=15)
    except Exception:
        return []
    ...
    for href in re.findall(
        ...
        # Bing иногда оборачивает ссылки редиректом.
        ...
        if not any(re.search(r"https?://([^/]*\.)?" + re.escape(d) + r"(/|$)", href, re.I)
                   for d in domains):
            continue

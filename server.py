...
def fantlab_cover_from_edition(edition_id):
    """Получает реальную обложку с карточки конкретного издания ФантЛаба."""
    if not edition_id:
        return ""
    url = f"https://fantlab.ru/edition{edition_id}"
    try:
        _, _, page = fetch_text(url, timeout=12)
    except Exception:
        return ""

    # На странице издания основная картинка обычно доступна через og:image.
    m = re.search(
    ...
    if m:
        return urllib.parse.urljoin("https://fantlab.ru/", m.group(1))

    # Запасной вариант для разметки страницы.
    for img in re.findall(
        ...
        if "image" in img.lower() or "cover" in img.lower():
            return urllib.parse.urljoin("https://fantlab.ru/", img)
...
def fantlab_search(query, limit=10):
    ...
    for x in data:
        ...
        cover = fantlab_cover_from_edition(edition_id)
        item = {
            "title": title,

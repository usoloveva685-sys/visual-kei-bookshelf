...
def bookvoed_search(q, limit=8):
    """Best-effort Bookvoed search with several URL forms and product parsing."""
    q = (q or "").strip()
    if not q:
        return []

    # The site's public search page is /search. Different deployments have
    # used different query parameter names, so try the common forms quickly.
    variants = [
        ...
        try:
            raw, _ = http_get(
                ...
                headers={
                    ...
                    "Referer": "https://www.bookvoed.ru/search"
                },
                timeout=7
            )
    ...
    for path in paths:
        ...
        try:
            ...
            # Prefer JSON-LD because it is much more stable than CSS markup.
            for jm in re.findall(
            ...
            # ISBN / author are often present in the visible product metadata.
            mi = re.search(r'ISBN\s*</[^>]+>\s*([^<]{10,20})', page, re.I)
            if not mi:
                mi = re.search(r'ISBN[^0-9Xx]{0,40}([0-9Xx][0-9Xx\- ]{9,20})', page, re.I)
            if mi:
                isbn = clean_isbn(mi.group(1))

            ma = re.search(r'Автор\s*</[^>]+>\s*([^<]{2,120})', page, re.I)
            if ma:
        ...
        # Fallback: use the slug as a title if the page metadata was blocked.
        if not title:

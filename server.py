...
def labirint_ids(q, limit=5):
    """Find Labirint product ids from its search HTML."""
    q = (q or "").strip()
    if not q:
        return []
    ...
    ids = []
    # Accept both normal and escaped forms that can occur in page markup.
    patterns = [
        r'href=["\']/books/(\d+)/["\']',
        r'/books/(\d+)/',
    ]
...
def labirint_search(q, limit=8):
    """Standalone Labirint search. Failure never affects other sources."""
    out = []
    for ident in labirint_ids(q, limit=limit):
        base = f"https://imo10.labirint.ru/books/{ident}/"
        out.append({
            "title": "",
            "author": "",
            "isbn": "",
            "cover": base + "cover.jpg/242-0",
...
def labirint_spine_for(q, isbn="", title="", author=""):
    ...
    for query in queries:
        hits = labirint_ids(query, limit=5)
        if hits:
            ident = hits[0]
            base = f"https://imo10.labirint.ru/books/{ident}/"
            return {
...
def enrich_labirint(results, original_q):
    ...
    for x in ranked:
        try:
            ...
            if hit:
                x.update(hit)
                # Prefer the actual Labirint cover when the original source
                # has no usable cover.
                if not x.get("cover"):
                    x["cover"] = hit.get("labirint_cover", "")
        except Exception:
            continue
    return results
...
def search(q):
    ...
    # Labirint is an independent source: if it fails, the other databases
    # remain fully usable.
    try:
        lab = labirint_search(q, limit=8)
        for x in lab:
            # Fill metadata/cover through the normal databases when possible.
            x["_score"] = score(q, x)
        allr.extend(lab)
    except Exception:
        pass

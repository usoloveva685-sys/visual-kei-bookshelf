...
def search_diagnostic(q):
    ...
    run_source("Читай-город", lambda query: chitai_search(query, limit=8))
        ...
        if u.path in ("/api/search", "/api/diagnostic"):
            ...
            try:
                ...
                if u.path == "/api/diagnostic":
                    payload["note"] = "Диагностика источников. Open Library намеренно отключён в этой тестовой версии."

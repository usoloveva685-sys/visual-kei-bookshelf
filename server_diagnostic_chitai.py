...
def normalize_chitai_title(title, author):
    """Очищает маркетинговые заголовки Читай-города и достаёт автора из скобок."""
    title = re.sub(r"\s+", " ", str(title or "")).strip()
    author = re.sub(r"\s+", " ", str(author or "")).strip()

    m = re.match(r"^(.*?)\s*\(([^()]+)\)\s*(?:купить книгу.*)?$", title, re.I)
    if m:
        base_title = m.group(1).strip()
        bracket_author = m.group(2).strip()
        if not author and bracket_author:
            author = bracket_author
        title = base_title

    title = re.sub(r"\s+(?:купить|заказать|цена|в наличии)\b.*$", "", title, flags=re.I)
...
def chitai_api_search(query, limit=10):
    ...
    try:
        ...
        raise RuntimeError("Читай-город API: " + str(exc)[:220])

    try:
        data = json.loads(raw)
    except Exception as exc:
        snippet = re.sub(r"\s+", " ", raw[:300]).strip()
        raise RuntimeError(
            "Читай-город API вернул не JSON: "
            + str(exc)[:100]
    ...
    if not results and isinstance(data, dict):
        top_keys = ", ".join(str(k) for k in data.keys())
        sample = ""
        for key in ("data", "included"):
            ...
            if isinstance(block, list) and block:
                if isinstance(block[0], dict):
                    sample = (
                        " | " + key + " item keys: "
                        + ", ".join(str(k) for k in block[0].keys())
                    )
                else:
                    sample = " | " + key + " is not object"
                break
        if candidates:
            types = ", ".join(sorted({
                str(x.get("type", ""))
            ...
            sample += " | types: " + (types or "none")
        raise RuntimeError(
            "Читай-город API: HTTP "
            + str(status)
            + ", но товары не распознаны; keys: "
            + top_keys
            + sample
        )
